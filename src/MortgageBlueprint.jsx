...
import VaResidualSummary from "./components/VaResidualSummary.jsx";
import { DEFAULT_VA_RESIDUAL, buildVaResidualView, vaResidualPdfData } from "./lib/vaResidual.js";
  ...
  <div style={{ display: "flex", alignItems: "center", background: T.inputBg, borderRadius: 12, padding: sm ? "10px 12px" : "12px 14px", border: focused ? `2px solid ${T.blue}` : `1px solid ${T.inputBorder}`, transition: "border 0.2s" }}>
   ...
   <input ref={inputRef} id={inputId} type="text" inputMode={inputMode || (isText ? "text" : "decimal")} pattern={pattern} autoComplete={autoComplete} readOnly={readOnly} value={display} onFocus={() => { if (readOnly) return; setFocused(true); wasFocused.current = true; setEditStr(null); }} onBlur={() => { setFocused(false); wasFocused.current = false; if (debounceRef.current) { clearTimeout(debounceRef.current); debounceRef.current = null; } if (editStr !== null) { const n = clamp(parseFloat(editStr.replace(/,/g, ""))); onChange(isNaN(n) ? 0 : n); setEditStr(null); } else if (!isText) { onChange(clamp(value)); } }} onChange={e => { if (isText) return onChange(e.target.value); const raw = e.target.value.replace(/,/g, ""); if (raw === "" || raw === "-") { setEditStr(""); if (debounceRef.current) clearTimeout(debounceRef.current); debounceRef.current = setTimeout(() => onChange(0), 300); return; } if (/^-?\d*\.?\d*$/.test(raw)) { const dotIdx = raw.indexOf("."); const intPart = dotIdx >= 0 ? raw.slice(0, dotIdx) : raw; const decPart = dotIdx >= 0 ? raw.slice(dotIdx) : ""; const fmtInt = intPart.replace(/^(-?)0+(\d)/, "$1$2").replace(/\B(?=(\d{3})+(?!\d))/g, ","); const formatted = fmtInt + decPart; const cursorPos = e.target.selectionStart; const commasBefore = (e.target.value.slice(0, cursorPos).match(/,/g) || []).length; const digitsBeforeCursor = cursorPos - commasBefore; let newCursor = 0, digitsSeen = 0; for (let i = 0; i < formatted.length; i++) { if (formatted[i] !== ",") digitsSeen++; if (digitsSeen >= digitsBeforeCursor) { newCursor = i + 1; break; } } if (digitsBeforeCursor === 0) newCursor = 0; cursorRef.current = newCursor; setEditStr(formatted); const n = parseFloat(raw); if (!isNaN(n)) { if (debounceRef.current) clearTimeout(debounceRef.current); debounceRef.current = setTimeout(() => onChange(n), 150); } } }} min={isText ? undefined : min} step={isText ? undefined : step}
    ...
    /* No negative tracking on numerals. -0.02em is display-headline styling;
       ...
       its decimal point, so "7.126" reads as though it's colliding (Christo
       2026-07-28). Tabular figures also do NOT inherit into form controls —
       browsers reset font properties on inputs — so the brand's tabular-nums
       rule has to be restated here or every input renders proportional. */
    style={{ background: "transparent", border: "none", outline: "none", color: T.text, fontSize: sm ? 15 : 17, fontWeight: isText ? 500 : 600, fontFamily: FONT, width: "100%", letterSpacing: "normal", fontVariantNumeric: "tabular-nums" }} />
...
export default function MortgageBlueprint({ initialState, borrowerMode }) {
 ...
 // (usually a seller/lender credit), NOT auto-applied to cash-to-close.
 const [buydownType, setBuydownType] = useState("none");
 const [buydownPaidBy, setBuydownPaidBy] = useState("seller"); // "seller" | "lender" | "borrower"
 ...
 // Blueprint loan number — sequential per device (BP-<year>-<0001>). The
 // counter lives in localStorage; a number is assigned once per scenario and
 // then persisted inside that scenario's saved state (getState/loadState).
 const [loanNumber, setLoanNumber] = useState("");
 const genLoanNumber = () => {
  try {
   const yr = new Date().getFullYear();
   const n = (parseInt(localStorage.getItem('bp_loan_seq') || '0', 10) || 0) + 1;
   localStorage.setItem('bp_loan_seq', String(n));
 ...
 // 3-Point Refi Test tab is LO-toggleable per scenario (doc 7.23). Default on
 // so existing scenarios keep the tab.
 const [showRefi3, setShowRefi3] = useState(true);
 ...
 // VA residual income (2026-09-13): the Form 26-6393 "balance available for
 // family support" test against the VA regional table. On by default for
 // every VA loan (null = auto); the Modules toggle can hide it (false).
 // Inputs persist per scenario as `vaResidual`; see lib/vaResidual.js.
 const [showVaResidual, setShowVaResidual] = useState(null);
 const [vaResidual, setVaResidual] = useState({ ...DEFAULT_VA_RESIDUAL });
 const vaResidualOn = loanType === "VA" && showVaResidual !== false;
 ...
 // Refi tax basis (Christo 2026-08-04): false = the county already reassessed
 // at their purchase, so the ACTUAL annual bill / 12 is the number. true =
 // bought within the last year (not yet reassessed) — estimate purchase-style.
 const [refiTaxAssessedMode, setRefiTaxAssessedMode] = useState(false);
 ...
 // often the whole reason for the refi.
 const [refiCurrentRateType, setRefiCurrentRateType] = useState("Fixed");
 // ARM history — lets the balance amortize truthfully instead of assuming one
 // constant rate (Christo 2026-07-22): the note starts at refiArmStartRate,
 // adjusts in refiArmAdjustedDate, and refiCurrentRate is the rate it
 // adjusted TO (the rate today).
 const [refiArmStartRate, setRefiArmStartRate] = useState(0);
 const [refiArmAdjustedDate, setRefiArmAdjustedDate] = useState("");
 // Statement anchors — the balance the servicer reports and the month of the
 // most recent payment. When present these beat the amortized estimate, and
 // the paid-through date is what per-diem payoff interest accrues from.
 const [refiLastPaymentDate, setRefiLastPaymentDate] = useState("");
 // Lender payoff fees — reconveyance, recording, doc prep, wire. Rolled into
 // the payoff amount; editable in Setup (doc 7.23).
 const [refiPayoffFees, setRefiPayoffFees] = useState(300);
 // "Will the closing-month payment be made before we close?" null = follow the
 // grace-day rule (auto); true/false = LO override. Closing after the 15th
 // (weekend-adjusted) means the payment is already due, so it MUST be made —
 // which drops the payoff principal and interest days (Christo 2026-07-22).
 const [refiClosingPmtOverride, setRefiClosingPmtOverride] = useState(null);
 // ── Two-flow intake (Christo 2026-07-28) ──────────────────────────────
 // One question decides everything after it: does the borrower have the
 // statement in front of them? Yes reads the loan, No reconstructs it. null =
 // not yet answered, which is what every pre-existing scenario loads as — the
 // old single flow is exactly the "yes" branch, so nulls fall through to it
 // and nothing saved before today changes shape.
 const [refiHasStatement, setRefiHasStatement] = useState(null);
 // Statements almost always print escrow as ONE line ("Escrow (Taxes and
 // Insurance)"), so combined is the default and the per-component split is the
 // exception. The combined figure is what the servicer COLLECTS today —
 // cushion and any shortage spread included — so it is deliberately NOT split
 // into refiAnnualTax/refiAnnualIns, which model the new loan's impound.
 const [refiEscrowMode, setRefiEscrowMode] = useState("combined");
 const [refiEscrowCombined, setRefiEscrowCombined] = useState(0);
 const [refiEscrowCombinedPeriod, setRefiEscrowCombinedPeriod] = useState("mo");
 // Second lien. A HELOC's rate is what makes it worth capturing: the new first
 // has to beat the BLENDED cost of both liens, not the first's rate alone.
 const [refiSecondLien, setRefiSecondLien] = useState(false);
 ...
 // Cash-out debt consolidation — "Paying off any other non-mortgage debt?"
 // null = unanswered. Yes surfaces the Debts-tab liabilities in the refi
 // setup; any debt marked "Yes - at Escrow" rolls its payoff into the new
 // loan and its retired payment onto the current side of every comparison.
 const [refiPayoffDebts, setRefiPayoffDebts] = useState(null);
 ...
 // Extra principal: a monthly amount needs only the amount, but a lump sum
 // needs a DATE — the same $20k in year two and year six leave very different
 // balances (Christo 2026-07-28).
 const [refiExtraCadence, setRefiExtraCadence] = useState("monthly"); // monthly | once
 const [refiExtraOnceDate, setRefiExtraOnceDate] = useState("");
 // Tri-state escrow knowledge for the reconstructed flow, where the borrower
 // genuinely may not know. "" = follow the yes/no booleans above.
 const [refiEscrowUnsure, setRefiEscrowUnsure] = useState("");
 // Maturity straight off the statement (Christo 2026-07-28). When the servicer
 // prints it, it beats anything derived: it survives past modifications and
 // recasts the closing-date estimate cannot see. "" = unanswered, which falls
 // through to the derived term, so nothing saved before today changes shape.
 const [refiHasMaturity, setRefiHasMaturity] = useState("");
 const [refiMaturityDate, setRefiMaturityDate] = useState("");
 // Homeowner's insurance effective (anniversary) month, "YYYY-MM-01" like
 // refiClosedDate. On a refi the policy carries over, so the 12-month premium
 // belongs in the closing costs ONLY when the renewal lands at closing —
 ...
 // Skipped payments are a function of the estimated closing date, not a
 // choice (Christo 2026-07-22): fund by the 15th and the next TWO payments
 // are skipped; fund after the 15th and only the next one is.
 React.useEffect(() => {
  if (isRefi) setRefiSkipMonths(closingDay <= 15 ? 2 : 1);
 }, [isRefi, closingDay]);
 ...
 const getState = () => ({
  ...
  transferTaxCity, discountPts, buydownType, buydownPaidBy, adminFee, lenderWireFee, underwritingFee, processingFee, appraisalFee, creditReportFee, floodCertFee, mersFee, taxServiceFee, titleInsurance, titleSearch, settlementFee, escrowFee, courierFee, loanTieInFee, notaryFee, envProtectionLien, recordingFee, lenderCredit, sellerCredit, realtorCredit, emd, emdPct, emdPaid, emdLocked, emdFlat,
 ...
 const loadState = (s) => {
  ...
  // Temporary buydown — default back to "none"/"seller" for older scenarios
  // that predate B2 so a stale selection can't leak across scenario switches.
  setBuydownType(s.buydownType !== undefined ? s.buydownType : "none");
  setBuydownPaidBy(s.buydownPaidBy !== undefined ? s.buydownPaidBy : "seller");
  ...
  // TYPE (a one-day option, 2026-07-22) — map those to Conventional/Adjustable.
  if (s.refiCurrentRateType !== undefined) setRefiCurrentRateType(s.refiCurrentRateType);
  if (s.refiArmStartRate !== undefined) setRefiArmStartRate(s.refiArmStartRate);
  if (s.refiArmAdjustedDate !== undefined) setRefiArmAdjustedDate(s.refiArmAdjustedDate);
  ...
  // Two-flow intake (2026-07-28). Every key is optional: a scenario saved
  // before today has none of them, loads with refiHasStatement null, and falls
  // through to the read-the-statement branch — which is what the old single
  // flow was. The one inference worth making is the escrow mode: a legacy
  // scenario that carries per-component amounts was entered split, so
  // defaulting it to "combined" would hide figures the LO already typed.
  if (s.refiHasStatement !== undefined) setRefiHasStatement(s.refiHasStatement);
  if (s.refiEscrowMode !== undefined) setRefiEscrowMode(s.refiEscrowMode);
  else if (s.refiAnnualTax > 0 || s.refiAnnualIns > 0) setRefiEscrowMode("split");
  if (s.refiEscrowCombined !== undefined) setRefiEscrowCombined(s.refiEscrowCombined);
  if (s.refiEscrowCombinedPeriod !== undefined) setRefiEscrowCombinedPeriod(s.refiEscrowCombinedPeriod);
  if (s.refiSecondLien !== undefined) setRefiSecondLien(s.refiSecondLien);
  ...
  // Same explicit-default rule: carrying the previous scenario's answer over
  // would silently roll one client's debts into another client's loan.
  setRefiPayoffDebts(s.refiPayoffDebts !== undefined ? s.refiPayoffDebts : null);
  ...
  if (s.refiExtraOnceDate !== undefined) setRefiExtraOnceDate(s.refiExtraOnceDate);
  if (s.refiEscrowUnsure !== undefined) setRefiEscrowUnsure(s.refiEscrowUnsure);
  if (s.refiHasMaturity !== undefined) setRefiHasMaturity(s.refiHasMaturity);
  if (s.refiMaturityDate !== undefined) setRefiMaturityDate(s.refiMaturityDate);
  ...
  if (s.showVaResidual !== undefined) setShowVaResidual(s.showVaResidual);
  if (s.vaResidual && typeof s.vaResidual === "object") setVaResidual({ ...DEFAULT_VA_RESIDUAL, ...s.vaResidual });
 ...
 useEffect(() => {
  ...
  // These were missing from the autosave deps — a lien-only (or statement/
  // maturity-only) edit never triggered a save until some other field moved.
  refiHasStatement, refiEscrowMode, refiEscrowCombined, refiEscrowCombinedPeriod,
  refiSecondLien, refiSecondKind, refiSecondBalance, refiSecondRate, refiSecondPlan, refiSecondPmtOverride,
  refiThirdLien, refiThirdKind, refiThirdBalance, refiThirdRate, refiThirdPlan, refiThirdPmtOverride,
  refiModified, refiPrepayPenalty, refiExtraCadence, refiExtraOnceDate, refiEscrowUnsure,
 ...
 const createScenario = async (name) => {
  ...
  setShowVaResidual(null); setVaResidual({ ...DEFAULT_VA_RESIDUAL });
  // Reset Prop 19
 ...
 const generateSummaryText = () => {
  ...
  if (isRefi) {
   ...
   sep();
   lines.push("SAVINGS");
   ln((c.refiLienPmtSaved > 0 || c.refiDebtPmtSaved > 0) ? "  Monthly P&I + Debt Savings" : "  Monthly P&I Savings", fmt(c.refiMonthlySavings));
   if (c.refiDebtPmtSaved > 0) ln("  Debt Payments Retired", fmt(c.refiDebtPmtSaved) + "/mo");
   ...
   ln((c.refiSecondPayoffAmt > 0 || c.refiThirdPayoffAmt > 0 || c.refiDebtPayoffTotal > 0) ? `  Payoffs (${c.refiLienLabel})` : "  Current Loan Payoff", "-" + fmt(c.refiNetPayoff));
 ...
 // Summary ⇄ Statement toggle. Statements are per-scenario documents in
 // private Supabase Storage (via the Ops API) — several per blueprint (e.g. a
 // first lien plus a HELOC), flipped between with chips, and readable by
 // Claude to autofill the current-loan fields.
 const [refiPreviewTab, setRefiPreviewTab] = useState('summary');
 const [stmtDocs, setStmtDocs] = useState({ status: 'idle', docs: [], activeId: null }); // idle|loading|ready|error
 const [stmtBlob, setStmtBlob] = useState({ id: null, blob: null, type: '' }); // active doc's bytes
 const [stmtBusy, setStmtBusy] = useState(''); // '' | 'uploading' | 'extracting'
 const [stmtFill, setStmtFill] = useState(null); // { lines: [], notes } after an autofill
 ...
 const [refiPreviewW, setRefiPreviewW] = useState(() => {
  ...
  try { return Math.max(560, Math.round(window.innerWidth * 0.52)); } catch { return 640; }
 });
 const [refiPreviewDragging, setRefiPreviewDragging] = useState(false);
 const startRefiPreviewResize = (e) => {
  e.preventDefault();
  setRefiPreviewDragging(true);
  const move = (ev) => setRefiPreviewW(Math.min(1500, Math.max(420, window.innerWidth - ev.clientX)));
  const up = () => {
 ...
 const buildWorksheetProps = () => ({
  ...
  // VA Residual Income page (VA loan + module on) — same snapshot pattern.
  vaResidual: vaResidualOn ? vaResidualPdfData(buildVaResidualView({ calc, isRefi, propertyState, married, taxState, vaResidual })) : null,
 });
 ...
 const buildWorksheetEmailBody = () => {
  ...
  const lines = [
   ...
   isRefi
    ? `• Monthly savings (P&I + MI): ${fmt(calc.refiPiMiSavings)}`
    : `• Estimated cash to close: ${fmt(calc.cashToClose)}`,
   ...(isRefi ? [`• Net cash in hand after closing: ${fmt(calc.refiNetCashInHand)}`] : []),
   `• Rate: ${rate}% (${loanType}, ${term}-year)`,
   "",
   // Refi emails follow Christo's working template (doc 7.23): next steps +
   // the lock-to-close timeline, so the borrower knows what happens when.
   ...(isRefi
    ? [
       "Next steps:",
       "1. Ask any questions.",
 ...
 const calc = useMemo(() => {
  ...
  const addDebt = (type, extra) => setDebts(prev => [...prev, { id: Date.now(), name: "", type, borrower: "Joint", balance: 0, monthly: 0, rate: 0, months: 0, payoff: "No", payoffAmount: 0, linkedReoId: "", ...extra }]);
  ...
  // ── Non-mortgage debts paid off through a cash-out refi (2026-08-25) ──
  // Debt consolidation is half the reason cash-out refis happen. The Debts
  // tab stays the single source of truth: on a cash-out refi where the LO
  // answered Yes to "paying off any other non-mortgage debt?", every debt
  // marked "Yes - at Escrow" rolls its payoff into the new loan and its
  // retired payment onto the CURRENT side of the cash-flow comparison.
  // Mortgage/HELOC-type debts are excluded — subject-property liens are the
  // 2nd/3rd lien model's job, other properties' liens are REO territory.
  const refiPaidDebts = (isRefi && refiPurpose === "Cash-Out" && refiPayoffDebts === true && !debtFree)
   ? debts.filter(d => d.payoff === "Yes - at Escrow" && d.type !== "Mortgage" && d.type !== "HELOC" && !reoLinkedDebtIds.has(d.id))
   : [];
  const refiPaidDebtIds = new Set(refiPaidDebts.map(d => d.id));
  const refiDebtPayoffTotal = refiPaidDebts.reduce((s, d) => s + (Number(d.payoffAmount) || Number(d.balance) || 0), 0);
  const refiDebtPmtSaved = refiPaidDebts.reduce((s, d) => s + (Number(d.monthly) || 0), 0);
  const refiDebtBalTotal = refiPaidDebts.reduce((s, d) => s + (Number(d.balance) || 0), 0);
  // Debts financed through the refi are NOT cash due at closing — they live in
  // the refi payoff walk instead. Payoff honors the per-debt payoffAmount when
  // one is entered (the Debts tab already did; this used to read balance only).
  const payoffAtClosing = debtFree ? 0 : debts.filter(d => d.payoff === "Yes - at Escrow" && !refiPaidDebtIds.has(d.id)).reduce((s, d) => s + (Number(d.payoffAmount) || Number(d.balance) || 0), 0);
  ...
  const refiCurMrEarly = (refiCurrentRate / 100) / 12;
  // On an ARM the note starts at refiArmStartRate and the payment RECASTS at
  // the adjustment: the balance re-amortizes over the remaining term at the
  // new rate. A constant-rate estimate is wrong on both sides of that date —
  // the reason Christo's real UWM file didn't match ("in Abe, it's not
  ...
  // The gate. null (every scenario saved before 2026-07-28) reads as "has a
  // statement", so legacy files keep the behaviour they were built with.
  const refiFromStatement = refiHasStatement !== false;
  // Statement figures only count in the branch that has a statement. State is
  // kept, not cleared, so flipping the gate back restores what was typed.
  const stmtBalance = refiFromStatement ? refiCurrentBalance : 0;
  const stmtPayment = refiFromStatement ? refiCurrentPayment : 0;
  const stmtPrin    = refiFromStatement ? refiCurPrinOverride : 0;
  const stmtInt     = refiFromStatement ? refiCurIntOverride : 0;
  ...
  // Extra principal, split by cadence (Christo 2026-07-28). A monthly amount
  // applies every month; a lump sum applies at ONE payment index, because the
  // same $20k in year two and year six leave very different balances. Indexed
  // in payments, so it carries the first-payment lag like armAdjMonth.
  const refiExtraMonthly = refiExtraCadence === "once" ? 0 : (refiExtraPaid || 0);
  const refiExtraLump = refiExtraCadence === "once" ? (refiExtraPaid || 0) : 0;
  const refiLumpMonth = (() => {
   if (refiExtraLump <= 0 || !refiExtraOnceDate || !refiClosedDate) return Infinity;
   const cd = new Date(refiClosedDate + "T00:00:00");
   const ld = new Date(refiExtraOnceDate + "T00:00:00");
   if (isNaN(cd) || isNaN(ld)) return Infinity;
   const cal = (ld.getFullYear() - cd.getFullYear()) * 12 + (ld.getMonth() - cd.getMonth());
   return Math.max(0, cal - REFI_FIRST_PMT_LAG - 1);
  })();
  const armSim = (() => {
   if (refiOriginalAmount <= 0 || refiCalcPI <= 0) return { balance: 0, pi: refiCalcPI };
   let bal = refiOriginalAmount;
   let pi = refiCalcPI;                       // start-rate payment
   let mr = armActive ? (refiArmStartRate / 100) / 12 : refiOrigMr;
   for (let m = 0; m < refiMonthsElapsed && bal > 0; m++) {
    if (m === armAdjMonth) {
     // Recast: remaining balance re-amortized over the remaining term at the
     // adjusted rate (= refiCurrentRate, the rate today).
     const remMonths = Math.max(1, refiOrigNp - m);
     pi = calcPI(bal, refiCurrentRate, remMonths / 12);
     mr = refiCurMrEarly;
    }
    const intPmt = bal * mr;
    bal -= (pi + refiExtraMonthly - intPmt);
    if (m === refiLumpMonth) bal -= refiExtraLump;   // one-time principal drop
   }
   // Adjustment lands exactly this month (loop ran m < elapsed, so index
   // armAdjMonth was never reached) — today's payment is still the recast one.
   if (armAdjMonth !== Infinity && refiMonthsElapsed === armAdjMonth) {
    pi = calcPI(bal, refiCurrentRate, Math.max(1, refiOrigNp - armAdjMonth) / 12);
   }
   return { balance: Math.max(0, bal), pi };
  })();
  const refiCalcBalance = armSim.balance;
  ...
  const refiEffPIBase = stmtPayment > 0
   ? stmtPayment
  ...
  const refiEffBalance = stmtBalance > 0
   ? stmtBalance
   : (refiOriginalAmount > 0 && refiClosedDate ? refiCalcBalance : 0);
  // Remaining term, most-trustworthy source first (Christo 2026-07-28):
  //   1. a maturity date printed on the statement — survives modifications and
  //      recasts that the closing-date estimate cannot see;
  //   2. the closing date plus the original term;
  //   3. a manually typed remaining count.
  const refiStatedMaturityMonths = (() => {
   if (refiHasMaturity !== "yes" || !refiMaturityDate) return null;
   const d = new Date(refiMaturityDate + "T00:00:00");
   if (isNaN(d)) return null;
   const now = new Date();
   return Math.max(0, (d.getFullYear() - now.getFullYear()) * 12 + (d.getMonth() - now.getMonth()));
  })();
  const refiEffRemaining = refiStatedMaturityMonths != null
   ? refiStatedMaturityMonths
   : (refiClosedDate ? refiCalcRemainingMonths : refiRemainingMonths);
  // The maturity we show: stated when we have it, otherwise projected from the
  // remaining count.
  const refiEffMaturity = (() => {
   const now = new Date();
   if (refiStatedMaturityMonths != null) return new Date(refiMaturityDate + "T00:00:00");
   if (!refiEffRemaining) return null;
   return new Date(now.getFullYear(), now.getMonth() + refiEffRemaining, 1);
  })();
  ...
  const refiCurSchedule = (() => {
   ...
   for (let m = 0; m < refiOrigNp && bal > 0.005; m++) {
    ...
    // Monthly extra every month; a lump sum only on the month it landed.
    const wantExtra = refiExtraMonthly + (m === refiLumpMonth ? refiExtraLump : 0);
    const extra = Math.min(wantExtra, Math.max(0, bal - prin));
  ...
  const refiScheduleNowBal = refiMonthsElapsed > 0
   ...
   : refiOriginalAmount;
  const refiStatementDelta = stmtBalance > 0 && refiCurSchedule.length > 0
   ? stmtBalance - refiScheduleNowBal
   : null;
  // ── Escrow as the statement prints it (Christo 2026-07-28) ──
  // Combined mode carries the servicer's single "Escrow (Taxes and Insurance)"
  // line. That is what they COLLECT — cushion and any shortage spread included
  // — so it drives the CURRENT payment only. It is deliberately never split
  // into tax and insurance: doing so would import someone else's shortage into
  // the new loan's impound, which is built from the assessor's bill and the
  // real premium instead.
  const refiEscrowCombinedMo = refiEscrowMode === "combined"
  ...
  const refiCurIntThisMonth = stmtInt > 0 ? stmtInt : refiEffBalance * refiCurMr;
  const refiCurPrinThisMonth = stmtPrin > 0 ? stmtPrin : Math.max(0, refiEffPIBase - refiCurIntThisMonth);
  const refiEffPI = (stmtPrin > 0 || stmtInt > 0)
   ? refiCurPrinThisMonth + refiCurIntThisMonth
   : refiEffPIBase;
  // Per-component escrow: tax and insurance can be impounded independently on
  // each loan (Christo 2026-07-22). Combined mode replaces both with the
  // servicer's single line.
  const refiCurEscrowMo = refiEscrowMode === "combined"
  ...
  // Payment is Principal + Interest + Escrow (+ MI), so P&I alone stops being
  // the reconciliation number the moment there is mortgage insurance or an
  // impound. Principal and interest stay itemised because only they amortize.
  const refiPaymentRows = (() => {
   const rows = [
    { label: "Principal", amt: refiCurPrinThisMonth },
    { label: "Interest", amt: refiCurIntThisMonth },
   ];
   if (refiCurrentMI > 0) {
    rows.push({ label: refiCurrentLoanType === "FHA" ? "Mortgage insurance (MIP)" : "Mortgage insurance (PMI)", amt: refiCurrentMI });
   }
  ...
  const refiPaymentTotal = refiPaymentRows.reduce((t, r) => t + r.amt, 0);
  // ── Remaining term solved from the statement itself ──
  // n = -ln(1 - r*B/P) / ln(1+r). Given the balance, the P&I split and the
  // rate, the term falls out with no original amount and no funded date — so
  // the maturity question is a cross-check, not a fork (Christo 2026-07-28).
  // Penny rounding in the servicer's own figures leaves a fractional
  // remainder (his loan solves to 355.0004); a bare ceil would invent a
  // phantom final payment, so snap to the nearest integer when within 0.02.
  const refiSolvedTerm = (() => {
   if (refiEffBalance <= 0 || refiEffPI <= 0) return null;
   if (refiCurMr <= 0) return Math.ceil(refiEffBalance / refiEffPI);
   const x = 1 - (refiCurMr * refiEffBalance) / refiEffPI;
   if (x <= 0) return null;                 // payment doesn't cover interest
   const raw = -Math.log(x) / Math.log(1 + refiCurMr);
   if (!isFinite(raw) || raw <= 0) return null;
   return Math.abs(raw - Math.round(raw)) < 0.02 ? Math.round(raw) : Math.ceil(raw);
  })();
  const refiSolvedMaturity = (() => {
   if (!refiSolvedTerm) return null;
   const now = new Date();
  ...
  // Blended rate weighs every balance the refi retires — liens AND consumer
  // debts being consolidated. A 24% card balance in the blend is exactly the
  // story a debt-consolidation refi tells: the new rate has to beat the
  // blended cost of everything, not the first's rate alone.
  const refiBlendedRate = ((refiSecondBal > 0 || refiThirdBal > 0 || refiDebtBalTotal > 0) && refiEffBalance > 0)
   ? (refiEffBalance * refiCurrentRate + refiSecondBal * (refiSecondRate || 0) + refiThirdBal * (refiThirdRate || 0)
      + refiPaidDebts.reduce((s, d) => s + (Number(d.balance) || 0) * (Number(d.rate) || 0), 0))
     / (refiEffBalance + refiSecondBal + refiThirdBal + refiDebtBalTotal)
   : refiCurrentRate;
  ...
  // under 80%, the answer is a cancellation request rather than a new loan.
  const refiPmiDropEligible = refiCurrentLoanType === "Conventional"
   && refiCurrentMI > 0 && refiHomeValue > 0 && refiEffBalance > 0
   && (refiEffBalance / refiHomeValue) < 0.80;
  // ── Confidence ──
  // Only things that undermine the NUMBER downgrade it. A modification breaks
  // a reconstruction but a statement balance is still ground truth, so it
  // degrades the estimated flow and not the read one. A prepayment penalty
  // changes the deal's economics, never the balance — it warns without
  // touching confidence.
  const refiUnsure = (() => {
   const u = [];
   if (!refiFromStatement) {
    if (refiEscrowUnsure === "unsure") u.push("whether taxes or insurance are impounded");
    if (refiExtraCadence === "unsure") u.push("whether extra principal was paid");
   }
  ...
  const refiBalanceSuspect = !refiFromStatement && (refiModified === "yes" || armActive);
  const refiConfidence = (refiUnsure.length || refiBalanceSuspect)
   ? "verify"
   : (refiFromStatement && stmtBalance > 0) ? "statement" : "estimated";
  ...
  // ── Payoff (the UWM-payoff-sheet math) ──
  // Balance + per-diem interest from the paid-through date to the estimated
  // closing date. Interest is paid in arrears (a payment made in month M pays
  // through the end of M-1; per-diem accrues from the 1st of M).
  //
  // Payments made before closing (Christo 2026-07-22, the Abe Diekman case):
  // every month strictly before the closing month is assumed paid (the loan
  // would be 30+ days late otherwise), PLUS the closing-month payment when
  // closing lands after the grace day. Grace day is the 15th, rolled to the
  // next business day if it falls on a weekend. Each assumed payment drops the
  // principal and moves the paid-through date forward, so both the payoff
  // principal AND its accrued-interest days shrink.
  const estCloseDateEarly = new Date(closingYear || new Date().getFullYear(), closingMonth - 1, closingDay);
  // Last payment defaults to the calendar, not a choice (doc 7.23): through
  // the 15th we assume last month's payment was the most recent; past the
  // 15th we assume this month's has been made. An explicit month wins.
  const refiLastPaymentEff = refiLastPaymentDate || (() => {
   const now = new Date();
   const d = new Date(now.getFullYear(), now.getMonth() - (now.getDate() <= 15 ? 1 : 0), 1);
   return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-01`;
  })();
  const refiGraceDay = (() => {
   const y = closingYear || new Date().getFullYear();
   const dow = new Date(y, closingMonth - 1, 15).getDay(); // 0 Sun .. 6 Sat
   return dow === 6 ? 17 : dow === 0 ? 16 : 15;            // weekend -> next business day
  })();
  const refiClosingPmtDue = closingDay > refiGraceDay;
  const refiAssumeClosingPmt = refiClosingPmtOverride == null ? refiClosingPmtDue : refiClosingPmtOverride;
  const refiMonthsToClose = (() => {
   if (!refiLastPaymentEff) return 0;
   const pt = new Date(refiLastPaymentEff + "T00:00:00");
   if (isNaN(pt)) return 0;
   const closeM = (closingYear || new Date().getFullYear()) * 12 + (closingMonth - 1);
   return Math.max(0, closeM - (pt.getFullYear() * 12 + pt.getMonth()));
  })();
  // Months strictly before the closing month are always paid; the closing
  // month itself only when assumed.
  const refiPaymentsBeforeClose = Math.max(0, refiMonthsToClose - 1) + (refiMonthsToClose >= 1 && refiAssumeClosingPmt ? 1 : 0);
  const refiPayoff = (() => {
   const bal0 = refiEffBalance;
   if (bal0 <= 0 || !refiLastPaymentEff) {
    return { balance: bal0, perDiem: 0, days: 0, interest: 0, amount: bal0, payments: 0, effLabel: "" };
   }
   const mr = (refiCurrentRate / 100) / 12;
   let bal = bal0;
   for (let i = 0; i < refiPaymentsBeforeClose && bal > 0; i++) {
    const intPart = bal * mr;
    bal = Math.max(0, bal - Math.max(0, refiEffPI - intPart)); // never negative-amortize on an unknown payment
   }
   const pt = new Date(refiLastPaymentEff + "T00:00:00");
   const effMonth = new Date(pt.getFullYear(), pt.getMonth() + refiPaymentsBeforeClose, 1);
   const days = Math.max(0, Math.round((estCloseDateEarly - effMonth) / 86400000));
   const perDiem = bal * (refiCurrentRate / 100) / 365;
   const interest = perDiem * days;
   return { balance: bal, perDiem, days, interest, amount: bal + interest + (Number(refiPayoffFees) || 0), payments: refiPaymentsBeforeClose,
            effLabel: effMonth.toLocaleDateString("en-US", { month: "long", year: "numeric" }) };
  })();
  const refiPayoffBalance = refiPayoff.balance;
  const refiPayoffPerDiem = refiPayoff.perDiem;
  ...
  // Secured debt only — the honest numerator for current LTV/equity rows.
  const refiCurSecuredDebt = refiEffBalance + refiLienPayoffBalTotal;
  // Consumer debts being consolidated join the "what the new loan replaces"
  // total (they're unsecured, so they stay OUT of refiCurSecuredDebt/LTV).
  const refiCurTotalDebt = refiCurSecuredDebt + refiDebtBalTotal;
  // "(1st + 2nd + 3rd + Debts)" label for every payoff-labeled surface.
  const refiLienLabel = ["1st", refiSecondPayoffAmt > 0 && "2nd", refiThirdPayoffAmt > 0 && "3rd", refiDebtPayoffTotal > 0 && "Debts"].filter(Boolean).join(" + ");
  ...
  // Cash-out builds the loan as payoff + consolidated debts + cash in hand —
  // refiCashOut stays "extra cash beyond the debt payoffs".
  const refiAutoLoanAmt = (refiPurpose === "Cash-Out" ? (refiPayoffAmount + refiCashOut + refiDebtPayoffTotal) : refiPayoffAmount) + refiSecondPayoffAmt + refiThirdPayoffAmt;
  ...
  // Bill-first: the county reassessed at their purchase, so the actual annual
  // bill / 12 is the truth. refiTaxAssessedMode (bought in the last year — not
  // reassessed yet) falls back to the purchase-style estimate.
  ...
  const refiMonthlySavings = isRefi ? ((refiEffPI + refiLienPmtSaved + refiDebtPmtSaved) - refiNewPi) : 0;
  ...
  // Consolidated debts' retired payments join the current side, same as
  // paid-off liens — that's the whole consolidation story.
  const refiPiMiSavings = isRefi ? ((refiEffPI + (Number(refiCurrentMI) || 0) + refiLienPmtSaved + refiDebtPmtSaved) - (refiNewPi + refiNewMI)) : 0;
  ...
  const refiMonthlyTotalSavings = isRefi ? ((refiCurCmpTotalPmt + refiLienPmtSaved + refiDebtPmtSaved) - refiNewTotalPmt) : 0;
  ...
  const refiNetPayoff = refiPayoffAmount + refiSecondPayoffAmt + refiThirdPayoffAmt + refiDebtPayoffTotal;
  const refiEstCashOut = refiNetNewLoan - refiNetClosingCosts - refiNetPrepaids - refiNetPayoff;
  // The skipped months skip the WHOLE outlay: when the second (or a
  // consolidated consumer debt) is being paid off at closing, its carry
  // stops too (Christo 2026-08-05).
  const refiSkipPmtAmt = (refiCurTotalPmt + refiLienPmtSaved + refiDebtPmtSaved) * (refiSkipMonths || 0);
  ...
  const refiCostOfWaiting = (() => {
   if (!isRefi || refiPiMiSavings <= 0) return [];
   ...
   return rateDrops.map(drop => {
    ...
    return {
     ...
     years: waitYears.map(y => {
      const lostSavings = refiPiMiSavings * y * 12;
  ...
  // (DTI) stays at the note rate, so nothing above changes. totalCost is a
  // seller/lender-concession data point; NOT folded into cash-to-close
  // (kept explicit — the UI notes it next to the total).
  const buydown = (buydownType !== "none" && loan > 0 && term > 0)
   ? calcTempBuydown(loan, rate, term, buydownType)
   : null;
  return {
   ...
   refiEscrowCombinedMo, refiEscrowOn, refiCurEscrowMo,
   refiPaymentRows, refiPaymentTotal,
   ...
   refiPaidDebts, refiDebtPayoffTotal, refiDebtPmtSaved, refiDebtBalTotal, refiCurSecuredDebt,
 ...
 // ── Mortgage statements: per-scenario documents behind the preview toggle ──
 ...
 useEffect(() => {
  ...
  if (stmtScenarioRef.current !== activeScenarioId) {
   ...
   setStmtDocs({ status: 'idle', docs: [], activeId: null });
   setStmtBlob({ id: null, blob: null, type: '' });
   setStmtFill(null);
   setStmtBusy('');
 ...
 // Fetch the document list the first time the panel needs it. Staleness is
 // judged against the scenario, NOT this effect's lifetime — the 'loading'
 // status change re-runs the effect, and an effect-scoped stale flag would
 // cancel our own in-flight fetch.
 useEffect(() => {
  if (!stmtAvailable || !refiPreviewOpen || !isRefi || !isDesktop) return;
  if (stmtDocs.status !== 'idle') return;
  const sid = activeScenarioId;
  setStmtDocs({ status: 'loading', docs: [], activeId: null });
  (async () => {
   try {
    const docs = await listStatements(sid);
    if (stmtScenarioRef.current !== sid) return;
    setStmtDocs({ status: 'ready', docs, activeId: docs[0]?.id || null });
   } catch (e) {
    if (stmtScenarioRef.current !== sid) return;
    console.warn('[Blueprint] statement list failed:', e.message);
    setStmtDocs({ status: 'error', docs: [], activeId: null });
   }
  })();
  // eslint-disable-next-line react-hooks/exhaustive-deps
 }, [stmtAvailable, refiPreviewOpen, isRefi, isDesktop, stmtDocs.status, activeScenarioId]);

 // Fetch the active document's bytes (signed URL → direct from storage).
 useEffect(() => {
  const id = stmtDocs.activeId;
  if (!stmtAvailable || !id || stmtBlob.id === id) return;
  const sid = activeScenarioId;
  (async () => {
   try {
    const r = await fetchStatementDoc(sid, id);
    if (stmtScenarioRef.current !== sid) return;
    setStmtBlob({ id, blob: r.blob, type: r.contentType });
   } catch (e) {
    if (stmtScenarioRef.current !== sid) return;
    console.warn('[Blueprint] statement download failed:', e.message);
    setStmtBlob({ id, blob: null, type: 'error' });
   }
  })();
  // eslint-disable-next-line react-hooks/exhaustive-deps
 }, [stmtAvailable, stmtDocs.activeId, stmtBlob.id, activeScenarioId]);

 // Photographed statements over ~5MB get downscaled; smaller ones and PDFs
 // upload as-is (the 10MB cap is enforced server-side too).
 ...
 const handleStatementFile = async (file) => {
  ...
  try {
   ...
   setStmtBusy('uploading');
   const sid = activeScenarioId;
   const { doc, upload_url } = await signStatementUpload(sid, name, type, blob.size);
   await putStatementToSignedUrl(upload_url, blob, type);
   if (stmtScenarioRef.current !== sid) return;
   setStmtDocs((p) => ({ status: 'ready', docs: [...p.docs, doc], activeId: doc.id }));
   setStmtBlob({ id: doc.id, blob, type });
 ...
 const handleStatementRemove = async () => {
  const id = stmtDocs.activeId;
  if (!stmtAvailable || !id) return;
  const doc = stmtDocs.docs.find((d) => d.id === id);
  if (!window.confirm(`Remove "${doc?.name || 'this statement'}" from the blueprint?`)) return;
  const prev = stmtDocs;
  const rest = stmtDocs.docs.filter((d) => d.id !== id);
  setStmtDocs({ status: 'ready', docs: rest, activeId: rest[0]?.id || null });
  if (!rest.length) setRefiPreviewTab('summary');
  try { await deleteStatementDoc(activeScenarioId, id); }
  catch (e) {
 ...
 // "Autofill" — Claude reads every uploaded statement and the extraction
 // lands in the refi current-loan fields. Only non-null values are applied,
 // and the banner lists exactly what was set.
 const handleStatementAutofill = async () => {
  if (!stmtAvailable || stmtBusy || !stmtDocs.docs.length) return;
  setStmtBusy('extracting');
  setStmtFill(null);
  try {
   const sid = activeScenarioId;
   const { extraction } = await extractStatements(sid);
   if (stmtScenarioRef.current !== sid) return;
   const lines = [];
   const money = (v) => `$${Number(v).toLocaleString('en-US', { maximumFractionDigits: 2 })}`;
   const num = (v) => typeof v === 'number' && isFinite(v) && v > 0;
   const f = extraction?.first_lien || {};
   setRefiHasStatement(true);
   if (num(f.principal_balance)) { setRefiCurrentBalance(f.principal_balance); lines.push(`Principal balance ${money(f.principal_balance)}`); }
   if (num(f.interest_rate_pct) && f.interest_rate_pct < 20) { setRefiCurrentRate(f.interest_rate_pct); lines.push(`Rate ${f.interest_rate_pct}%`); }
   if (num(f.regular_payment_total)) { setRefiCurrentPayment(f.regular_payment_total); lines.push(`Monthly payment ${money(f.regular_payment_total)}`); }
   if (num(f.principal_amount)) { setRefiCurPrinOverride(f.principal_amount); lines.push(`Principal portion ${money(f.principal_amount)}`); }
   if (num(f.interest_amount)) { setRefiCurIntOverride(f.interest_amount); lines.push(`Interest portion ${money(f.interest_amount)}`); }
   if (num(f.escrow_amount)) {
    setRefiEscrowCombined(f.escrow_amount);
    setRefiEscrowCombinedPeriod('mo');
    setRefiEscrowMode('combined');
    setRefiCurEscrowTax(true);
    setRefiCurEscrowIns(true);
    lines.push(`Escrow (tax & ins) ${money(f.escrow_amount)}/mo`);
   }
   if (num(f.mortgage_insurance_amount)) { setRefiCurrentMI(f.mortgage_insurance_amount); lines.push(`Mortgage insurance ${money(f.mortgage_insurance_amount)}/mo`); }
   if (num(f.escrow_balance)) { setRefiEscrowBalance(f.escrow_balance); lines.push(`Escrow balance ${money(f.escrow_balance)}`); }
   if (typeof f.maturity_date === 'string' && /^\d{4}-\d{2}$/.test(f.maturity_date)) {
    setRefiHasMaturity('yes');
    setRefiMaturityDate(`${f.maturity_date}-01`);
    lines.push(`Maturity ${f.maturity_date}`);
   }
   if (f.loan_type && f.loan_type !== 'unknown') {
    setRefiCurrentLoanType(f.loan_type === 'conventional' ? 'Conventional' : f.loan_type.toUpperCase());
    lines.push(`Loan type ${f.loan_type.toUpperCase()}`);
   }
   const s = extraction?.second_lien;
   if (s && (num(s.balance) || num(s.interest_rate_pct))) {
    setRefiSecondLien(true);
    setRefiSecondKind(s.kind === 'fixed' ? 'closed' : 'heloc');
    if (num(s.balance)) { setRefiSecondBalance(s.balance); lines.push(`2nd lien balance ${money(s.balance)}`); }
   ...
   console.warn('[Blueprint] statement autofill failed:', e.message);
 ...
 const stmtImgUrl = useMemo(
  () => (stmtBlob.blob && stmtBlob.type !== 'application/pdf' && stmtBlob.type !== 'error')
   ? URL.createObjectURL(stmtBlob.blob) : null,
  [stmtBlob]
 );
 ...
 <Sec title="Loan Comparison">
  <Card>
   ...
   {refiHomeValue > 0 && [
    ...
    // Secured debt only — consolidated credit cards aren't liens, so they
    // don't belong in the current LTV/equity numerator.
    ["LTV", pct(refiHomeValue > 0 ? calc.refiCurSecuredDebt / refiHomeValue : 0, 1), pct(calc.refiNewLTV, 1)],
    ["Equity", fmt(refiHomeValue - calc.refiCurSecuredDebt), fmt(refiHomeValue - calc.refiNewLoanAmt)],
   ].map(([l, c, n], i) => (
  ...
  <Card>
   ...
   {(() => {
    ...
    const rows = [
     ...
     // Consolidated consumer debts — one aggregate row at the bottom
     // (Christo 2026-08-25): the payments they're making today vs $0 after
     // the refi pays them off. THE debt-consolidation line.
     ...((calc.refiPaidDebts || []).length > 0 ? [{
      label: "Other Debts",
      cur: calc.refiDebtPmtSaved,
      nw: 0,
      note: `${calc.refiPaidDebts.length} debt${calc.refiPaidDebts.length === 1 ? "" : "s"} paid off at closing`,
     }] : []),
   ...
   {(() => {
    ...
    const curTotalAll = (refiShowTaxIns
     ...
     : calc.refiEffPI + (Number(refiCurrentMI) || 0)) + (calc.refiLienPmtCur || 0) + (calc.refiDebtPmtSaved || 0);
 ...
 <Sec title="Net Cash Out">
  <Card>
   ...
   <MRow label={(calc.refiSecondPayoffAmt > 0 || calc.refiThirdPayoffAmt > 0 || calc.refiDebtPayoffTotal > 0) ? `− Payoffs (${calc.refiLienLabel})` : "− Current Loan Payoff"} value={`-${fmt(calc.refiNetPayoff)}`} sub={(calc.refiSecondPayoffAmt > 0 || calc.refiThirdPayoffAmt > 0 || calc.refiDebtPayoffTotal > 0) ? [`${fmt(calc.refiPayoffAmount)} first`, calc.refiSecondPayoffAmt > 0 && `${fmt(calc.refiSecondPayoffAmt)} second`, calc.refiThirdPayoffAmt > 0 && `${fmt(calc.refiThirdPayoffAmt)} third`, calc.refiDebtPayoffTotal > 0 && `${fmt(calc.refiDebtPayoffTotal)} debts`].filter(Boolean).join(" + ") : undefined} />
   <div style={{ borderTop: `2px solid ${T.separator}`, marginTop: 8, paddingTop: 8 }}>
   ...
   {(calc.refiSkipPmtAmt > 0 || calc.refiEscrowRefund > 0) && <>
    ...
    {calc.refiSkipPmtAmt > 0 && <MRow label={`Skip ${refiSkipMonths} Payment${refiSkipMonths > 1 ? "s" : ""}`} value={`+${fmt(calc.refiSkipPmtAmt)}`} color={T.green} sub={`${refiSkipMonths} × ${fmt(calc.refiCurTotalPmt + calc.refiLienPmtSaved + calc.refiDebtPmtSaved)}/mo${calc.refiLienPmtSaved > 0 || calc.refiDebtPmtSaved > 0 ? " incl. paid-off debts" : ""}`} />}
 ...
 const TABS = [["overview","Overview"],
  ...
  ...(isRefi ? [["refi","Refi Summary"], ...(showRefi3 ? [["refi3","3-Point Test"]] : [])] : []),
 ...
 const OVERVIEW_SECTIONS = [
  ...
  ...(vaResidualOn ? [{ id: "overview-varesidual", label: "VA Residual" }] : []),
  ...
  {isRefi && refiPreviewOpen && isDesktop && (
   <div style={{ position: "fixed", top: 96, right: 0, bottom: 0, width: refiPreviewW, zIndex: 900, display: "flex", flexDirection: "column", background: darkMode ? "rgba(10,17,32,0.88)" : "rgba(240,246,252,0.92)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderLeft: `1px solid ${T.glassBorder || T.separator}`, boxShadow: darkMode ? "-8px 0 30px rgba(0,0,0,0.35)" : "-8px 0 30px rgba(10,17,32,0.10)", overflow: "hidden" }}>
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "12px 16px 8px 20px", flexShrink: 0 }}>
     ...
     <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      {refiPreviewTab === 'statement' && stmtAvailable && stmtDocs.docs.length > 0 && (
       <>
        <button onClick={handleStatementAutofill} disabled={!!stmtBusy} title="Read the statements and prefill the current-loan fields" style={{ background: stmtBusy === 'extracting' ? `${T.blue}1f` : T.blue, border: "none", borderRadius: 9999, color: stmtBusy === 'extracting' ? T.blue : "#fff", cursor: stmtBusy ? "wait" : "pointer", fontSize: 10, fontWeight: 600, padding: "3px 12px", fontFamily: FONT }}>{stmtBusy === 'extracting' ? 'Reading…' : 'Autofill'}</button>
        <button onClick={() => stmtFileInputRef.current?.click()} disabled={!!stmtBusy} title="Upload another statement" style={{ background: "none", border: `1px solid ${T.separator}`, borderRadius: 9999, color: T.textSecondary, cursor: "pointer", fontSize: 10, fontWeight: 600, padding: "3px 10px", fontFamily: FONT }}>Add</button>
        <button onClick={handleStatementRemove} disabled={!!stmtBusy} title="Remove the shown statement" style={{ background: "none", border: `1px solid ${T.separator}`, borderRadius: 9999, color: T.textSecondary, cursor: "pointer", fontSize: 10, fontWeight: 600, padding: "3px 10px", fontFamily: FONT }}>Remove</button>
       </>
      )}
    ...
    {/* Statement chips — flip between the blueprint's documents (e.g. first
        lien + HELOC). Shown only when more than one is uploaded. */}
    {refiPreviewTab === 'statement' && stmtAvailable && stmtDocs.docs.length > 1 && (
     <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 20px 8px", flexWrap: "wrap", flexShrink: 0 }}>
      {stmtDocs.docs.map((d) => (
       <button key={d.id} onClick={() => setStmtDocs((p) => ({ ...p, activeId: d.id }))} title={d.name} style={{
        background: stmtDocs.activeId === d.id ? `${T.blue}1f` : "none",
        border: `1px solid ${stmtDocs.activeId === d.id ? T.blue : T.separator}`,
        borderRadius: 9999, color: stmtDocs.activeId === d.id ? T.blue : T.textTertiary,
        cursor: "pointer", fontSize: 10, fontWeight: 600, padding: "3px 10px", fontFamily: FONT,
    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "4px 20px 24px" }}>
     {refiPreviewTab === 'statement' && stmtAvailable ? (
      <>
       {/* Autofill result banner — exactly what Claude read and set. */}
       {stmtFill && (
        <div style={{ background: `${T.blue}12`, border: `1px solid ${T.blue}40`, borderRadius: 12, padding: "10px 14px", marginBottom: 12, fontFamily: FONT }}>
         <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ flex: 1, fontSize: 11, fontWeight: 700, color: T.blue }}>
           {stmtFill.lines.length ? `Filled ${stmtFill.lines.length} field${stmtFill.lines.length !== 1 ? 's' : ''} from the statement${stmtDocs.docs.length > 1 ? 's' : ''}` : 'Nothing filled'}
          </div>
          <span onClick={() => setStmtFill(null)} style={{ cursor: "pointer", color: T.textTertiary, fontSize: 13 }}>&times;</span>
         </div>
         {stmtFill.lines.length > 0 && (
          <div style={{ fontSize: 11, color: T.text, marginTop: 6, lineHeight: 1.6 }}>
           {stmtFill.lines.join(' · ')}
          </div>
         )}
         {stmtFill.notes && (
          <div style={{ fontSize: 10.5, color: T.textTertiary, marginTop: 6, lineHeight: 1.5 }}>{stmtFill.notes}</div>
         )}
        </div>
       )}
       {stmtBusy === 'extracting' && (
       ...
       {stmtDocs.status === 'ready' && stmtDocs.docs.length > 0 ? (
        stmtBlob.id === stmtDocs.activeId && stmtBlob.blob ? (
         stmtBlob.type === 'application/pdf'
          ? <RefiPdfPagesPreview blob={stmtBlob.blob} width={refiPreviewW - 47} darkMode={darkMode} />
          : (
        ...
        ) : stmtBlob.id === stmtDocs.activeId && stmtBlob.type === 'error' ? (
         <div style={{ display: "grid", placeItems: "center", height: "60%" }}>
          <div style={{ textAlign: "center", color: T.textTertiary, fontSize: 12, fontFamily: FONT }}>
           ...
           <div style={{ marginTop: 8 }}>
            <button onClick={() => setStmtBlob({ id: null, blob: null, type: '' })} style={{ background: "none", border: `1px solid ${T.separator}`, borderRadius: 9999, color: T.textSecondary, cursor: "pointer", fontSize: 11, fontWeight: 600, padding: "4px 12px", fontFamily: FONT }}>Retry</button>
           </div>
          </div>
         </div>
        ) : (
         <div style={{ display: "grid", placeItems: "center", height: "60%", color: T.textTertiary, fontSize: 12, fontFamily: FONT }}>Loading statement…</div>
        )
       ) : stmtDocs.status === 'loading' || stmtBusy === 'uploading' ? (
        <div style={{ display: "grid", placeItems: "center", height: "60%", color: T.textTertiary, fontSize: 12, fontFamily: FONT }}>
         {stmtBusy === 'uploading' ? 'Uploading statement…' : 'Loading statements…'}
        </div>
       ) : stmtDocs.status === 'error' ? (
        <div style={{ display: "grid", placeItems: "center", height: "60%" }}>
         <div style={{ textAlign: "center", color: T.textTertiary, fontSize: 12, fontFamily: FONT }}>
          Couldn't load the statements.
          <div style={{ marginTop: 8 }}>
           <button onClick={() => setStmtDocs({ status: 'idle', docs: [], activeId: null })} style={{ background: "none", border: `1px solid ${T.separator}`, borderRadius: 9999, color: T.textSecondary, cursor: "pointer", fontSize: 11, fontWeight: 600, padding: "4px 12px", fontFamily: FONT }}>Retry</button>
          </div>
         </div>
        </div>
       ) : (
...
{tab === "summary" && (<>
 ...
 {vaResidualOn && <Sec title="VA Residual Income">
  <VaResidualSummary T={T} calc={calc} isRefi={isRefi} propertyState={propertyState} married={married} taxState={taxState} vaResidual={vaResidual} Card={Card} />
 </Sec>}
...
{tab === "overview" && (
 <Suspense fallback={<div style={{ padding: 40, textAlign: "center", color: T.textTertiary }}>Loading Overview...</div>}>
  <OverviewTab {...{
   ...
   /* Refi Quick Start asks for the subject address instead of a bare ZIP */
   propertyAddress, setPropertyAddress, setPropertyTBD, addressInput, setAddressInput, AddressAutocomplete,
   ...
   showVaResidual, setShowVaResidual, vaResidualOn, vaResidual, setVaResidual,
   ...
   refiCurrentLoanType, setRefiCurrentLoanType, refiCurrentRateType, setRefiCurrentRateType, refiArmStartRate, setRefiArmStartRate, refiArmAdjustedDate, setRefiArmAdjustedDate, refiLastPaymentDate, setRefiLastPaymentDate, refiClosingPmtOverride, setRefiClosingPmtOverride, closingMonth, setClosingMonth, closingDay, setClosingDay, closingYear, setClosingYear,
   ...
   refiPayoffDebts, setRefiPayoffDebts,
   /* Live rates */
...
<Suspense fallback={null}>
 ...
 <BottomSheet isOpen={sheetContent === "debts"} onClose={() => setSheetContent(null)} title="Debts & Liabilities" T={T}>
  <DebtsSheet
   debts={debts} addDebt={calc.addDebt}

...
export default function AmortContent(props) {
  ...
  if (import.meta.env.DEV) devCheckProps("AmortContent", props, ["T", "isDesktop", "calc", "fmt", "extraPayment", "setExtraPayment", "amortView", "setAmortView", "term", "rate", "salesPrice", "appreciationRate", "setAppreciationRate", "isPulse", "markTouched", "Hero", "Card", "Inp", "Tab", "MRow", "AmortChart", "GuidedNextButton"]);
  const {
  ...
  // Equity moved out of the tab strip to always show on the left
  // (Christo 2026-09-23); a scenario saved on the old "equity" tab lands on
  // the monthly table instead.
  const view = amortView === "equity" ? "monthly" : amortView;
  const hasExtra = (Number(extraPayment) || 0) > 0;
  // Opaque green panel: tint composited over the solid card so the
  // blueprint canvas doesn't read through (the old 8%-alpha fill over the
  // 50% glass card was hard to read and "distracting" — Christo 2026-09-23).
  const greenPanel = `linear-gradient(${T.green}1f, ${T.green}1f), ${T.card}`;
 const equityBlock = (() => {
  ...
  return (<>
   <Card pad={14}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 8, flexWrap: "wrap" }}>
     ...
     <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", background: T.inputBg, borderRadius: 8, overflow: "hidden" }}>
   ...
   {last && (
    <Card style={{ marginTop: 8, background: greenPanel, border: `1px solid ${T.green}40` }}>
 ...
 <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, margin: isDesktop ? "0 0 12px" : "16px 0" }}>
  <Card pad={14} style={{ height: "100%", boxSizing: "border-box", marginBottom: 0 }}>
  ...
  {/* Extra payment — the amount alone drives it (toggle retired
      2026-09-23: $0 already means "no extra payment"). */}
  <div data-field="amort-section" className={isPulse("amort-section")} onFocus={() => markTouched("amort-section")} onClick={() => markTouched("amort-section")} style={{ borderRadius: 14, transition: "all 0.3s" }}>
  <Card pad={14} style={{ height: "100%", boxSizing: "border-box", marginBottom: 0 }}>
   <Inp label="Monthly Extra Payment" value={extraPayment} onChange={setExtraPayment} sm tip="Additional principal paid each month beyond your required payment. Reduces total interest and shortens your loan term. Leave at $0 for none." />
  </Card>
  </div>
 </div>
 {hasExtra && calc.intSaved > 100 && (
  <Card style={{ background: greenPanel, border: `1px solid ${T.green}40` }}>
 ...
 <div style={{ marginBottom: 12 }}>{equityBlock}</div>
 ...
 <div style={{ display: "flex", gap: 4, marginBottom: 12, flexWrap: "wrap" }}>
  {["monthly","yearly"].map(v => <Tab key={v} label={v.charAt(0).toUpperCase() + v.slice(1)} active={view === v} onClick={() => setAmortView(v)} />)}
  <button onClick={() => {

...
export default function CostsContent({
  ...
  // ────────────────────────────────────────────────────────────
  // Local UI state: which rows have their inline editor expanded
  // ────────────────────────────────────────────────────────────
  ...
  const AriveBox = ({ title, total, children, accent = AMBER, highlightTotal = true }) => (
    <div style={{
      ...
      <div style={{
        ...
        {total !== undefined && (
          <div style={{ fontSize: 14, fontWeight: 700, fontFamily: MONO, color: highlightTotal ? accent : T.text }}>
            {total}
          </div>
        )}
      </div>
  ...
  // FeeRow — label + amount, optional inline edit expansion
  const FeeRow = ({ label, sub, value, editKey, onChange, suffix = null, prefix = "$", bold = false, color, step = 1, max, isDollar = true, note, readOnly = false }) => {
      ...
      <div style={{ borderBottom: `1px dashed ${T.separator}`, paddingBottom: 2 }}>
        <div style={{
          ...
          <div style={{
            ...
            fontFamily: MONO,
            color: color || (bold ? T.text : T.text),
            flexShrink: 0,
            marginLeft: 10,
          }}>{displayVal}</div>
        </div>
  ...
  // ─── Derived numbers used across boxes ────────────────────────
  const proposedHOI_12mo = annualIns;
  ...
  const thirdPartyTotal = calc.cannotShop + calc.canShop + (isRefi ? 0 : (ownersTitleIns + homeWarranty + (hoa > 0 ? (hoaTransferFee > 0 ? hoaTransferFee : hoa) : 0) + calc.buyerCommAmt));

  // P&I and monthly pieces
  ...
  return (<>
    ...
    <div style={isDesktop ? { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, alignItems: "flex-start" } : {}}>
      ...
      <AriveBox title="Taxes and Other Government Fees" total={fmt2(calc.govCharges)}>
        ...
        {openRow === "transferTax" && !isRefi && (
          <div style={{ padding: "4px 0 10px 26px" }}>
            <Sel
              ...
              tip="Government tax when property changes hands. Rate varies by city."
            />
        ...
        {openRow === "transferTax" && isRefi && (
          <Note color={T.blue}>No transfer tax on refinances in California.</Note>
        )}
      </AriveBox>
      ...
      <AriveBox title="Third Party Fees" total={fmt2(thirdPartyTotal)}>
        ...
        {!isRefi && (
          <>
            ...
            <div style={{ padding: "8px 0", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px dashed ${T.separator}` }}>
              ...
              >
                <div style={{
                  ...
                  position: "absolute", top: 2, left: buyerPaysComm ? 18 : 2,
      ...
      <AriveBox title="Prepaids and Initial Escrow Payment at Closing" total={fmt2(calc.totalPrepaidExp)}>
        ...
        {!includeEscrow ? (
          <Note color={T.orange}>Escrow waived — no initial escrow collected at closing.</Note>
        ) : (
          <>
      ...
      <AriveBox title={isRefi ? "Estimated Refi Costs" : "Estimated Funds To Close"} total={undefined}>
        ...
        <FeeRow
          ...
          editKey="lenderCredit"
          value={lenderCredit}
          onChange={setLenderCredit}
        />
        {!isRefi && (
          <>
            <FeeRow
              label="Seller Credits"
              editKey="sellerCredit"
              value={sellerCredit}
              onChange={setSellerCredit}
            />
            <FeeRow
              label="Realtor Credit"
              editKey="realtorCredit"
              value={realtorCredit}
              onChange={setRealtorCredit}
            />
          </>
        )}
    ...
    {/* First-payment explainer — keep below the grid, unchanged vibe */}
    {(() => {
